...
def load_resources():
    ...
    ort_session = ort.InferenceSession(config.ONNX_MODEL_PATH, sess_options, providers=['CPUExecutionProvider'])
    ...
    while True:
        ...
        if res.detections:
            if len(res.detections) > 1: msg, color = "⚠️ MULTIPLE FACES", (0, 0, 255)
                ...
                if stability >= config.STABILITY_FRAMES:
...
def generate_enrollment_frames(name):
    ...
    while p_idx < len(phases):
        ...
        if res.detections:
            ...
            if cur_p == target:
                if is_well_lit(crop) and is_sharp(crop) > config.BLUR_THRESHOLD:
                ...
                else: color, msg = (0, 0, 255), "QUALITY LOW"
    ...
    if len(embs) == 125:
        ...
        for i, p in enumerate(p_names):
            ...
            normalized_centroid = np.mean(chunk, axis=0)
            normalized_centroid /= np.linalg.norm(normalized_centroid)
            np.save(os.path.join(config.DB_PATH, f"{name}_{p}.npy"), normalized_centroid)
        load_resources()
