...
CSV_FILE = "benchmark_stable_smoothed.csv"
THRESHOLD = 0.60              # Adjusted for stability
STABILITY_FRAMES = 8          
BUFFER_SIZE = 12              # Increased for smoother identity transition
SMOOTHING_FACTOR = 0.20       # [NEW] 0.2 means 20% new box, 80% old box (Higher = Faster, Lower = Smoother)
...
class IdentityStabilizer:
    ...
    def update(self, name):
        ...
        # Only switch if the new name appears in >50% of the buffer
...
def get_embedding_onnx(img_bgr):
    """Extract embedding with verification window"""
...
# [NEW] Smoothed Bounding Box Variable
prev_box = None 
...
while cap.isOpened():
    ...
    if results.detections:
        ...
        if stability_counter < STABILITY_FRAMES:
            ...
            if face_crop.size > 0:
                ...
                raw_user = best_match_name if best_score > THRESHOLD else "Unknown"
        ...
        stability_counter = 0
        prev_box = None # Reset smoothing if person leaves frame
    ...
    cv2.imshow("Smoothed Recognition System", frame)
